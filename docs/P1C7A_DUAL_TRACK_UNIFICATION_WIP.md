...
## 📅 第 5 会话执行计划 (2026-09-10 · 目标: 一天四件事搞大更新)

> 前置状态: 基线干净可复现 (af=6.25/dmg=1746.2 s3串行20局, commit b967162)
> 总预算: ~8h (每项含验证, 负回归即回退止损, 不恋战)

### T1 · B 方案迁移 (核心, ~2.5h) — 唯一有不确定性的任务

| 步骤 | 内容 | 验收 |
|---|---|---|
| 1 | weapons.json fist recovery 0.35→**0.5** (range 保持 1.5) | JSON 改动 |
| 2 | player_controller 删 legacy 空手分支 (v3 同款 diff, 已验证可编译) | 编译过 |
| 3 | 冒烟: s3 串行 20 局 | 对照基线 af/dmg, **逐字节最优**, ±5% 内可接受 |
| 4 | 聚合: 5 种子×100 局 (p1c7a_summary.py) | af≥1.60·dmg≥196 附近即过 |
| 5a | **过** → 直接进 T2 收尾 | — |
| 5b | **不过** → 二分找行为差: ①combo.hit 双写 (v3 删除过, 零行为差已证) ②VFX rng 抽号顺序 ③EventBus 事件差异; 每个探针 30 分钟止损 | 定位或放弃 (记 C 方案) |

注意: v3 已证 recovery=0.35 是负回归, B 方案唯一变量 = 0.5。若 0.5 也
不过, 则 executor 轨存在未知行为差, 当天可宣布迁移失败并转 C 方案
(MCTS 感知补丁) — 不再消耗会话。

### T2 · 迁移落地收尾 (依赖 T1 通过, ~1h)

| 步骤 | 内容 | 验收 |
|---|---|---|
| 1 | sim_ai build_sim_state 改读 `weapon.can_attack` 感知统一 (一行版已在会话2验证) | MCTS 感知同轨 |
| 2 | 删 `Player::can_attack` + `ATTACK_COOLDOWN` 死代码 (全调用点先 grep) | 编译过 + 全测试过 |
| 3 | 复跑冒烟 20 局确认感知改动零行为差 | af/dmg 与 T1 后一致 |
| 4 | mirror_combat_director 523/575 行的 `_last_attack_time` 观察**保留** (executor 路径仍写该字段, 语义不变) | 复核 |

### T3 · P1-C7-C Boss reset 语义拆分 (~2.5h, 与 T1 无依赖可穿插)

| 步骤 | 内容 | 验收 |
|---|---|---|
| 1 | grep `BossSystemDirector` 全部成员字段 → 按"局内跨层须持久 vs 跨局须清"两列分类 | 分类表 |
| 2 | 拆分 API: `reset_floor()` (跨层清) vs `reset_run()` (跨局清) | 接口清晰 |
| 3 | 补调用点: enter_floor → reset_floor; new_game → reset_run | 全路径覆盖 |
| 4 | 冒烟 20 局 s3 + 单局×5 种子 (基线对照 — 本修复应零行为差, 因为基线从未调用过 reset) | **逐字节一致或仅时间戳差** |
| 5 | 若有行为差: boss 行为记忆跨层残留本来就在基线里"工作着" (乱拳老师效应), 逐字段评估是否真该清 | 证据决策 |

已知难点: replay_mem 语义 (局内跨层须持久 vs 跨局须清) — 拆分时最容易
翻车的字段, 单独写单元测试。

### T4 · spear 局尾 special 探针 + 文档收尾 (~1h)

| 步骤 | 内容 | 验收 |
|---|---|---|
| 1 | per-run 探针: WeaponSpecialState 激活中局结束的频率 + 下局开局 active 残留检查 | 数据 |
| 2 | 若残留为 0 → 从嫌疑清单划掉记档; 若非 0 → 定位 new_game 清理缺口修复 | 判决 |
| 3 | WIP 文档 → 结论文档 (P1C7A 结项报告), CHANGELOG v1.4.12 | 文档 |
| 4 | README 里程碑/CHANGELOG 同步 + 桌面打包版同步 (exe 复制到桌面根目录) | 桌面 |

### 当日止损线

- T1 两次迭代仍负回归 → 转记 C 方案 (MCTS 补丁), T2 跳过, 时间让给 T3
- T3 分类有争议字段 → 保守: 只拆明确项, 争议字段记档下批
- 一切以"负回归即回退"为铁律 — 本项目 P1-C5/C6/C7 已 8 次验证此铁律
