...
(FOV 红雾等基础材质统一后再做, 防特效大杂烩) → M6-m 角色资产升级
(先 4 个 Hero Asset: 玩家/F15 Echo/一普通怪/一精英, 验证自然再批量)
→ M6-n 视觉收口 (门贴墙/描边/光照参数/性能回归)
...
- Arena 物件 (爆炸桶/图腾) / Boss FOV 红雾 / 事件房间 ? 标记未做 (M6-k/l)
