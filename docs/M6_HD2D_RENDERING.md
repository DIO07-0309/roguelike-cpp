...
- ~~bloom 三档为手调常量, 未做逐帧亮度反馈~~ ⚠️ v1.6-A4 实装后实机
  验收无可见变化 (岩浆桶代理粒度不足, 火山层饱和 → lum≡center),
  已按退避预案回退手调三档 (见 A4 节尾注)
- **墙顶"镂空" = 顶面对比度不足, 非几何缺失** (v1.6-A3.2-fix2 结案):
  A3.2 顶面 = 墙面同贴图×1.1, 深渊墙面本身暗 → 顶面暗同虚空。
  修复: builder 按 2D 同源 wall_top/wall_face 通道比映射顶面 tint
  (实测顶/侧亮度比 3.22); 顶面 quad 本身一直在光栅化 (debug 实证)
- **未探索区 = 不绘制 (虚空)** (v1.6-A3.2-fix2 设计变更): 原 i.1
  "暗色岩石块"从俯视泄露走廊/房间轮廓; 跳过后探索区浮于黑暗
  (战争迷雾语义)。未探索岩浆随之不入点光表 (不再漏光)。
  存档不保存探索标记 = 2D 既有设计, 载入后迷雾重置, 两模式一致
...
> **⚠️ 2026-09-17 已回退**: 实机验收"看不出任何变化" — 岩浆桶代表光数
> 粒度不足 (典型火山层可见桶数饱和 → lum≡center, 反馈通道无信号)。
> 按退避预案移除 EMA 反馈, 恢复 v2g 手调三档; 本节设计记录保留备查。
> 若重启此方向, 需先解决代理信号灵敏度 (如按屏内岩浆 tile 面积加权)。
