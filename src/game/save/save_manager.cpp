...
bool SaveManager::save_game(Player* player, int floor, int max_f) {
    ...
    for (auto& item : inv.items) {
        ...
        if (eq && eq->slot != "charm") {
            ...
            fprintf(f, "%s,%d,%s,%d;",
                cn->base_name.c_str(), (int)cn->rarity, cn->effect_type.c_str(), cn->effect_value);
        }
    }
    fprintf(f, "\n");
    ...
    fclose(f);
    LOG_INFO("存档: 第%d层 Lv%d HP:%d/%d %zu技能 %zu物品",
...
SaveData* SaveManager::load_save() {
    ...
    if (!inv_s.empty()) {
        for (size_t pos = 0; pos < inv_s.size(); ) {
            ...
            if (typ == "heal" && parts.size() >= 4) {
                p->inventory.items.push_back(
                    std::make_shared<ConsumableItem>(nm, rar, typ, atoi(parts[3].c_str())));
    ...
    LOG_INFO("读档: 第%d层 Lv%d HP:%d/%d %zu技能 %zu物品",
        floor, lv, chp, mhp,
