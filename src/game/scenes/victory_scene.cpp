#include "victory_scene.h"
#include "title_scene.h"
#include "scene_tree.h"
#include "core/logger.h"

extern Font g_font, g_font_small;
extern bool g_font_loaded;

void VictoryScene::_render() {
    ClearBackground(BLACK);
    int sw = get_tree()->get_width(), sh = get_tree()->get_height();
    if (g_font_loaded) {
        float w = MeasureTextEx(g_font, "恭喜通关！", 60, 1).x;
        DrawTextEx(g_font, "恭喜通关！", {sw/2.0f - w/2, 100}, 60, 1, {50, 255, 100, 255});
        char buf[64];
        snprintf(buf, sizeof(buf), "最终等级 Lv%d", final_level);
        w = MeasureTextEx(g_font_small, buf, 18, 1).x;
        DrawTextEx(g_font_small, buf, {sw/2.0f - w/2, 180}, 18, 1, {200, 240, 200, 255});
        w = MeasureTextEx(g_font, "按 Enter 返回标题", 22, 1).x;
        DrawTextEx(g_font, "按 Enter 返回标题",
                   {sw/2.0f - w/2, (float)(sh - 60)}, 22, 1, {255, 200, 50, 255});
    } else {
        DrawText("VICTORY!", sw/2 - 60, 110, 48, {50, 255, 100, 255});
        DrawText("Press Enter", sw/2 - 50, 180, 18, {200, 240, 200, 255});
    }
}

void VictoryScene::_input(const InputMap& input) {
    if (input.is_action_just_pressed("confirm")) {
        auto ts = std::make_shared<TitleScene>();
        ts->name = "TitleScene";
        get_tree()->change_scene(ts);
        LOG_INFO("通关→返回标题");
    }
}
