...
> **v1.4.0** — C++17 + Raylib 5.0 | CMake | **300+ 源文件** | Windows 实机验证
>
...
>
> **v1.3.x**：游戏定名「回响深渊 Abyssal Echo」· 主界面大字号重排 · 11 步新手教程（三卡片元素选择 + 捡/用/穿/打真实闭环）· 6 条首遇提示 · 窗口可缩放 + letterbox 3:2 + 鼠标逻辑坐标 · MinGW 运行库静态链接（exe 自包含）
>
> **v1.4.0 · G10.9 多存档系统**：**三存档槽（Slot API：`get_all_slots`/`get_slot_summary`/`set_active_slot`，UI 层零接触存档格式）** · 统一选档界面（新游戏/继续/选关三模式复用，键鼠统一）· 满档删除二次确认（仅删档、Meta 永不波及）· **结局收集迁账号级 Meta**（删档不丢全收集）+ best_floor 历史最高 · 镜像记忆随档（三档=三种 Build 三套镜像进化）· 存档 v:5（版本号首次真正读取 + play_time 时长字段 + 二进制模式）· **旧档一键安全迁移**（save.json→slot_1.json 字节级验证 + .bak 保留）· HUD 存档徽标 · **修复 4 个历史 Bug**（继续游戏覆盖 Meta 清零 / 结局解锁从不落盘 / 死亡 runs 双计 / 通关奖励双发）· 10 项槽位隔离测试（60/60 全绿）
